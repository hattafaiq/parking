...
void utama::on_button_ticket_clicked()
{
    ...
    QDateTime waktu_cek;
    //int info_plat_ada = db.cek_in_avaliablelity(0, "W39370GSR",&waktu_cek);
    int info_plat_ada = db.cek_in_avaliablelity(jenis_kendaraannya, plat_nomor,&waktu_cek,0);
    if(info_plat_ada){
        qDebug()<<"plat nomor sama, tidak dapat masuk";
    }
    else {
...
void utama::on_button_pay_clicked()
{
   ...
   //masukkan data
   int sukses_kah = db.insert_default_cek_out(id_cek_in, foto, foto, waktu_now, price);
   qDebug()<<"cek sukses cek out:"<<sukses_kah;
